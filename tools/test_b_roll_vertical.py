...
def run_b_roll_test():
    ...
    # Test Prompt
    prompt_text = "Cinematic close-up of a vintage fountain pen resting on textured paper, sunbeams hitting dust motes, cozy study room, warm afternoon lighting, 50mm f/1.8, vertical, bokeh, 8k, high fidelity."
    
    # 1. Update Positive Prompt (Node 45)
    if "45" in workflow:
        workflow["45"]["inputs"]["text"] = prompt_text
        print(f"Set prompt: {prompt_text}")

    # 2. Randomize Seed (Node 44)
    if "44" in workflow:
        seed = random.randint(1, 9999999999)
        workflow["44"]["inputs"]["seed"] = seed
        print(f"Set seed: {seed}")

    # 3. Set Filename Prefix (Node 48)
    if "48" in workflow:
        workflow["48"]["inputs"]["filename_prefix"] = "TEST_B_ROLL"
    ...
    print(f"Sending B-Roll workflow to RunPod Endpoint {ENDPOINT_ID}...")
            ...
            if status == 'COMPLETED':
                ...
                if output and 'images' in output:
                    for img in output['images']:
                        ...
                        filename = f"outputs/test_b_roll_{job_id}.png"
